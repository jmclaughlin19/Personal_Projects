...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	if (XSpi_Transfer(&SpiInstance, send, NULL, 2) != 0) {
	     xil_printf("MAXreg_wr error occurred\n");
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (XSpi_Transfer(&SpiInstance, buff, buff, 1+nbytes) != 0) {
		        xil_printf("MAXreg_rd error occurred\n");
		    }
...
BYTE MAXreg_rd(BYTE reg) {
	...
	buf[1] = 0;
	    ...
	    XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	    if (XSpi_Transfer(&SpiInstance, buf, buf, 2) != 0) {
	        xil_printf("MAXreg_rd error occurred\n");
	    }
	    XSpi_SetSlaveSelect(&SpiInstance, 0x00);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (int i = 1; i < (nbytes+1); i++) {
	 ...
	 if (XSpi_Transfer(&SpiInstance, buf, buf, 1+nbytes) != 0) {
		       xil_printf("MAXbytes_rd error occurred");
	 }
