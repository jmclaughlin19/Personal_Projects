...
class KDTree
{
  ...
  private:
    ...
    vector<Point<Dim>> points_vec;

    KDTreeNode * TreeMake(int left, int right, int dimension);

    void copy_helper(KDTreeNode* &cur,KDTreeNode* &other);

    void delete_helper(KDTreeNode* & subroot);

    Point<Dim> neighbor_helper(KDTreeNode* subroot, const Point<Dim>& query, int dimension) const;

    // template <typename RandIter>
    // KDTreeNode* TreeMake(RandIter begin, RandIter end, int dimension);
